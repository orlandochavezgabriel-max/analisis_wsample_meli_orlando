...
def calcular_volumenes_y_pallets(df_programacion, df_pct_envio, df_kpis):
    ...
    for fase in ['inbound', 'storing', 'outbound']:
        col_vol_fase = 'vol_storing' if fase == 'storing' else f'volumen_{fase}'
        df_programacion[col_vol_fase] = df_programacion['volumen'] * df_programacion[f'pct_{fase}']
        
        df_programacion[f'volumen_small_{fase}'] = df_programacion[col_vol_fase] * pct_small
        df_programacion[f'volumen_heavy_{fase}'] = df_programacion[col_vol_fase] * pct_heavy
...
def calcular_oferta_meli_por_mediana(df_horario, matriz_cob):
    """Calcula el HC sugerido por turno dividiendo la demanda entre turnos concurrentes."""
...
if __name__ == "__main__":
    ...
    archivo_salida = 'Reporte_Consolidado_Planificacion_MELI_rev02.xlsx'
