...
# ==============================================================================
# BLOQUE 5: COBERTURA, BALANCEO Y EXTRACCIÓN MODULAR DE FASES
# ==============================================================================
...
def calcular_oferta_meli_por_mediana(df_horario, matriz_cob):
    """Calcula el HC sugerido por turno considerando el empalme de turnos por mediana."""
...
def procesar_escenario_operativo(vol_mod, pct_df):
    """Genera el dataset central Detalle_HC_Diaristas_Hora con todos los cálculos integrados."""
    resultados_horarios = []
    for dia in vol_mod['dia'].unique():
        ...
        # Aseguramos conservar el volumen diario asociado a cada hora
        df_dia['volumen'] = vol_dia
        
        # 1. Inbound Subprocesos
    ...
    return df_horario, df_resumen_turnos, matriz_cob


# ==============================================================================
# BLOQUE 6: EXTRACCIÓN MODULAR Y LIMPIA DESDE EL MASTER DETALLE
# ==============================================================================
def extraer_datos_fase_por_hojas(df_detalle_master, matriz_cob, col_demanda_fase, nombre_fase):
    """Extrae y calcula la alícuota, oferta y brecha específica de una fase leyendo el master."""
    ...
    for t_idx in range(matriz_cob.shape[1]):
        horas_activas = matriz_cob[:, t_idx] == 1
        if np.sum(horas_activas) > 0:
            mediana_turno = np.median(demanda_alicuota[horas_activas])
            hc_sugerido = int(np.ceil(mediana_turno))
        else:
            hc_sugerido = 0
        hc_turnos_fase.append(hc_sugerido)
        
    hc_turnos_fase = np.array(hc_turnos_fase)
    oferta_fase = np.dot(matriz_cob, hc_turnos_fase)
    gap_fase = (df_detalle_master[col_demanda_fase] - oferta_fase).clip(lower=0)
    
    df_detalle_fase = pd.DataFrame({
        'Día': df_detalle_master['dia'],
        'Hora': df_detalle_master['hora'],
        'Volumen (paq/h)': df_detalle_master['volumen'],
        f'HC Demanda {nombre_fase}': df_detalle_master[col_demanda_fase],
        'Turnos Activos': turnos_activos_hora,
        'HC Oferta Fija MELI': oferta_fase,
        f'HC Diaristas Requeridos ({nombre_fase})': gap_fase
    })
...
if __name__ == "__main__":
    print("=== INICIANDO MODELO DE PLANIFICACIÓN LOGÍSTICA MELI (MULTIDETALLE) ===")
    ...
    df_detalle_hora, df_resumen_turnos, matriz_cob = procesar_escenario_operativo(vol_prog, pct_prog)
    
    fases_a_procesar = {
        'Inbound': df_detalle_hora['hc_descarga'] + df_detalle_hora['hc_conexion_in'],
        'Sorting': df_detalle_hora['hc_sorting'],
        'Outbound': df_detalle_hora['hc_conexion_out'] + df_detalle_hora['hc_despachador']
    }
    
    hojas_fases_data = {}
    for nombre_fase, serie_demanda in fases_a_procesar.items():
        df_detalle_hora[f'_demanda_{nombre_fase}'] = serie_demanda
        det_fase, res_fase = extraer_datos_fase_por_hojas(
            df_detalle_hora, matriz_cob, f'_demanda_{nombre_fase}', nombre_fase
        )
        hojas_fases_data[f'Detalle_{nombre_fase}'] = det_fase
        hojas_fases_data[f'Resumen_Turnos_{nombre_fase}'] = res_fase
        df_detalle_hora.drop(columns=[f'_demanda_{nombre_fase}'], inplace=True)
    ...
    print(f"\n¡Proceso completado exitosamente sin duplicidad de datos!")
